...
class Logger {
  ...
  outputToConsole(level, message, context, error) {
    ...
    switch (level) {
      case 'ERROR':
        originalConsole.error(prefix, message, context, error);
        break;
      case 'WARN':
        originalConsole.warn(prefix, message, context);
        break;
      case 'INFO':
        originalConsole.info(prefix, message, context);
        break;
      case 'DEBUG':
        originalConsole.debug(prefix, message, context);
        break;
      case 'TRACE':
        originalConsole.trace(prefix, message, context);
        break;
      default:
        originalConsole.log(prefix, message, context);
    }
  }
  ...
  persistLogs() {
    try {
      ...
      originalConsole.error('Failed to persist logs:', error);
    }
  }
  ...
  loadLogs() {
    try {
      ...
      originalConsole.error('Failed to load logs:', error);
    }
  }
...
// TEMPORARILY DISABLED: Console method override to prevent recursive errors
...
// Make originalConsole available globally for other utilities
window.originalConsole = originalConsole;

// Prevent recursive logging
let isLogging = false;

// TEMPORARILY DISABLED: Console method overrides to prevent recursion
/*
console.log = (...args) => {
  if (!isLogging) {
    isLogging = true;
    try {
      ...
      originalConsole.error('Logger error:', error);
    } finally {
      isLogging = false;
    }
  }
